...
from .models import Faq,AQIRange,LiteralSettings,NumericSettigs,Concentrations,MeasureData,MeasureDataView,AQINowCastHistory,History,Measurements,BackgroundTaskModel,Measurements
...
class ConcentrationsAdmin(admin.ModelAdmin):
    list_display = ['real_start','start','final','sum','count','data']
    list_per_page = 15
    list_filter = ['real_start','start','final']
...
class MeasurementsAdmin(admin.ModelAdmin):
    ...
    def has_delete_permission(self, request,obj=None):
        return False
    list_display = ['author','start_timestamp','current_timestamp']
    list_per_page = 15
...
class connectionRequestAdmin(admin.ModelAdmin):
    ...
    def start_record(self,request):
        ...
        if s is not None:
            ...
            if not task_created:
                if 'Running' in device_status:

                    measure = Measurements(
                    author=models.User.objects.get(request.user) ,
                    ...
                    messages.add_message(request,messages.ERROR, 'El dusttrak no se encuentra en modo Running. Favor, inicie el dispositivo y luego inicie el proceso de grabacion ')
            else:
...
class MyAdminSite(AdminSite):
    site_url = 'http://mediciones.aire.uc.edu.py'
    def index(self, request, extra_context=None):
        ...
        context = dict(
            ...
            link_stop_device='/polls/history/stopdevice',
            link_stop_record='/polls/history/stoprecord',
            link_start_device='/polls/history/startdevice',
            link_start_record='/polls/history/startrecord'
